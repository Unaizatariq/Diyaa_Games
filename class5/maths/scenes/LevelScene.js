...
import { getClass5MathsLevel } from "../data/levels.js";
...
export default class LevelScene extends CurriculumScene {
    constructor() {
        const meta = getClass5MathsLevel(window.DIYAA_GAME_CONFIG?.level || 1);
    ...
    buildQuestions() {
        ...
        if (m === "place-value") return Array.from({ length: 8 }, () => { const n = Phaser.Math.Between(10000, 99999); return { prompt: `Value of digit ${String(n)[2]} in ${n}?`, answer: Number(String(n)[2]) * 100, options: s([Number(String(n)[2]) * 100, Number(String(n)[1]) * 1000, Number(String(n)[3]) * 10, Number(String(n)[4])]), hint: "Check the hundreds place." }; });
        ...
        if (m === "multiplication") return Array.from({ length: 8 }, () => { const a = Phaser.Math.Between(10, 99), b = Phaser.Math.Between(2, 12), ans = a * b; return { prompt: `${a} x ${b} = ?`, answer: ans, options: s([ans, ans + b, ans - b, ans + 10]), hint: "Break into tens and ones." }; });
        if (m === "division") return Array.from({ length: 8 }, () => { const b = Phaser.Math.Between(2, 12), ans = Phaser.Math.Between(10, 60), a = b * ans; return { prompt: `${a} / ${b} = ?`, answer: ans, options: s([ans, ans + 1, ans - 1, ans + 2]), hint: "Check by multiplication." }; });
        if (m === "factors-multiples") return [[12, "factor"], [18, "factor"], [24, "multiple"], [30, "multiple"], [16, "factor"], [20, "multiple"]].map(([n, kind]) => ({ prompt: kind === "factor" ? `Which is a factor of ${n}?` : `Which is a multiple of ${n}?`, answer: kind === "factor" ? "2" : String(n * 2), options: kind === "factor" ? ["2", "5", "7", "9"] : [String(n * 2), String(n + 3), String(n - 1), String(n + 7)], hint: "Factor divides exactly; multiple is n x k." }));
        ...
        if (m === "geometry") return [["Acute angle", "< 90 deg"], ["Right angle", "90 deg"], ["Obtuse angle", "> 90 deg"], ["Triangle", "3 sides"], ["Quadrilateral", "4 sides"]].map(([x, a]) => ({ prompt: `${x} has`, answer: a, options: s([a, "2 sides", "180 deg", "circle"]), hint: a }));
